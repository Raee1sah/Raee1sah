# Hi, I'm Raaidah Ahmad 👋

## Data Scientist | ML Engineer | Race Performance Analyst

I'm a Data Science student with real-world experience in machine learning, data analysis, and Python development. I've processed over 500,000 data points, built predictive models that improved customer retention by 15%, and trained 100+ students in Python and data science.

### 🔧 Technical Skills
- **Languages & Tools:** Python, SQL, BigQuery
- **ML & AI:** Machine Learning, Neural Networks, PyTorch, Scikit-learn
- **Data Visualization:** Power BI, Tableau, Seaborn, Matplotlib
- **Cloud & Platforms:** AWS, Microsoft Azure

### 💼 Professional Experience
- **Junior Data Scientist** at FlipRobo Technologies - Built predictive models and automated data pipelines
- **Python Trainer** at Zion Tech Hub - Trained 100+ students in data science
- **Founder & Data Strategist** at Husna Étoile - Grew online sales by 60% using data analytics

### 🎯 Key Achievements
- Improved customer retention by **15%** through behavioral modeling
- Automated data pipelines, reducing processing time by **40%**
- Analyzed and processed **500,000+ data points** for business insights

### 📚 Current Focus
- Completing B.Sc. in Data Science at International University of Applied Sciences (2026)
- Building end-to-end ML projects
- Exploring advanced analytics and model deployment
- Learning all about Vehicle Dyamics and Rcace Strategy

### 📫 Let's Connect
- Email: raaidahahmad51@gmail.com
- LinkedIn: [linkedin.com/in/raaidah-ahmad](https://linkedin.com/in/raaidah-ahmad)
- Location: Lagos, Nigeria

---

💡 **Check out my pinned repositories below to see my work!**
